...
export default function MarketDetail({ market, onBack }:
  ...
  return (
    <div className="p-4 space-y-4">
      <button onClick={onBack} className="text-sm text-sky-400">← Markets</button>
      <h1 className="text-lg font-semibold leading-snug">{market.question}</h1>
      <div className="flex gap-2 text-sm">
        <span className="px-2 py-1 rounded-full bg-emerald-500/15 text-emerald-300">YES {asOdds(market.yesPrice)}</span>
        <span className="px-2 py-1 rounded-full bg-rose-500/15 text-rose-300">NO {asOdds(market.noPrice)}</span>
        {market.url && (
          <a href={market.url} target="_blank" rel="noreferrer"
             className="ml-auto text-sky-400 text-sm">Open on Polymarket ↗</a>
        )}
      </div>
      ...
      <section className="rounded-2xl bg-slate-900 p-3 space-y-2">
        ...
        <div className="flex rounded-xl bg-slate-800 p-1">
          {(["YES","NO"] as const).map((s) => (
            <button key={s} onClick={() => setSide(s)}
              className={`flex-1 py-1.5 rounded-lg text-sm ${side === s ? "bg-sky-500 text-white" : "text-slate-300"}`}>{s}</button>
          ))}
        </div>
        <label className="flex items-center gap-2 text-sm">
          ...
          <input inputMode="decimal" value={qty} onChange={(e) => setQty(e.target.value)}
                 className="flex-1 rounded-xl bg-slate-800 border border-slate-700 px-3 py-2"/>
        </label>
        ...
        <button onClick={() => setRecording(true)}
          className="w-full rounded-xl bg-sky-500 py-2 font-medium">
      ...
      <section className="rounded-2xl bg-slate-900 p-3 space-y-1 text-sm">
