    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-3 border-t border-slate-800 flex items-center gap-2">
        <button onClick={() => { setCmdMode(!cmdMode); db.setSettings({ commandMode: !cmdMode }); }}
          className={`px-2 py-1 rounded-lg text-xs ${cmdMode ? "bg-sky-500" : "bg-slate-800"}`}
        ...
        <textarea
          ...
          rows={1}
          placeholder={cmdMode ? "(sell) Market Outcome N" : "Say something… or record a trade"}
          className="flex-1 resize-none rounded-xl bg-slate-900 border border-slate-800 px-3 py-2 text-sm"
        />
        <button onClick={send} disabled={busy || !draft.trim()}
          className="rounded-xl bg-sky-500 px-3 py-2 text-sm font-medium disabled:bg-slate-700">
          Send
        </button>
      </div>
    </div>
...
function Bubble({ msg, onConfirm }: { msg: Msg; onConfirm: (t: ParsedTrade) => void }) {
  ...
  return (
    <div className={`flex ${isUser ? "justify-end" : "justify-start"}`}>
      <div className={`max-w-[85%] rounded-2xl px-3 py-2 ${isUser ? "bg-sky-500 text-white" : "bg-slate-800 text-slate-100"}`}>
        <div className="text-sm whitespace-pre-wrap">{msg.text}</div>
        {msg.pending && (
          <div className="mt-2 flex gap-2">
            <button onClick={() => onConfirm(msg.pending!)}
              className={`flex-1 rounded-lg py-1 text-sm font-medium ${
                msg.pending.side === "BUY" ? "bg-emerald-500" : "bg-rose-500"
